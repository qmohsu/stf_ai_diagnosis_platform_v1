# Copied from diagnostic_api/tests/harness/evals/judge.py @ 9c9e7a9 (PROD-10).
...
from stf_v3.evals.judge_prompts import (
    JUDGE_SYSTEM_PROMPT,
    build_user_prompt,
    classify_pitfall_directive,
)
from stf_v3.evals.metrics import (
...
def _build_default_client() -> AsyncOpenAI:
    ...
    Reads the V3 cloud key (``settings.cloud_api_key``: the
    OpenRouter key) and ``cloud_llm_base_url`` (OpenRouter).  Raises if no API key is configured so eval runs
    ...
    Raises:
        RuntimeError: If no OpenRouter key is configured.
    """
    if not settings.cloud_api_key:
        raise RuntimeError(
            "Judge requires STF_V3_OPENROUTER_API_KEY (or "
            "STF_V3_CLOUD_LLM_API_KEY) in the environment.",
        )
    return AsyncOpenAI(
