...
/**
 * /goldens/obd listing page (HARNESS-21 [2b/4]).
 *
 * Mirrors /goldens/manual structurally but with OBD-lane
 * buckets (6 question_types) and lane="obd" in the API call.
 * Reuses the same review-status badge convention.
 *
 * Author: Li-Ta Hsu
 */
...
import {
  OBD_BUCKETS,
  type GoldenEntrySummary,
  type GoldenReviewStatus,
  type OBDGoldenBucket,
} from "@/lib/types";
...
export default function ObdGoldensListingPage() {
  const { t } = useTranslation();
  ...
  useEffect(() => {
    ...
    listGoldens({
      ...
      bucket:
        bucketFilter === "all"
          ? undefined
          : (bucketFilter as OBDGoldenBucket),
      limit: 200,
    })
  ...
  const grouped = useMemo(() => {
    const out: Record<OBDGoldenBucket, GoldenEntrySummary[]> = {
      signal_statistics: [],
      event_finding: [],
      dtc_enumeration: [],
      dtc_decode: [],
    ...
    for (const item of items) {
      const bucket = item.question_type as OBDGoldenBucket;
      if (bucket in out) {
    ...
    <div className="container mx-auto px-4 py-6 space-y-4">
      <div className="flex items-center gap-2">
        ...
        >
          ...
          {t("goldens.obdListing.backToLanding")}
        </Link>
      </div>
      ...
      <Card>
        <CardHeader className="space-y-2">
          <CardTitle className="flex items-center gap-2">
            {t("goldens.obdListing.title")}
          </CardTitle>
          <p className="text-sm text-muted-foreground">
            {t("goldens.obdListing.description")}
          </p>
                  ...
                  {bucketItems.length === 0 ? (
                    ...
                    <ul className="space-y-2">
                      {bucketItems.map((item) => (
                        <li key={item.id}>
                          <Link
                            href={`/goldens/obd/${encodeURIComponent(item.id)}`}
