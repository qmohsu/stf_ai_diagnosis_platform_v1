...
GitHub Issue: #97.
Status: **PR [2a/4] COMPLETE** (2026-05-24) — eval-side fixes + 15 real Yamaha goldens landed.  PR series rescoped 3→4 PRs after a post-[1/3] discussion (see v1.5.3 of `v2_design_doc.md`).  PR [2b/4] adds the OBD UI lane + v1→v2-tier migration; PR [3/4] runs the baseline + iterates prompts.  Design doc: `docs/plans/2026-05-17-harness-21-obd-eval-design.md`.
...
4. **PR [3/4] — Baseline + iterate** — real-LLM run on PolyU + `docs/harness_21_phase5_baseline.md`; threshold raised from 0.6 once baseline known; optional `obd_agent_prompts.py` tweaks driven by failures.  Possibly probes `"think": false` extra_body for Qwen as a follow-up to PR [2a]'s timeout bump if latency variance is still wide.
