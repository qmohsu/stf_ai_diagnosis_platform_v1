...
3. **Generator + reviewer scripts** ✅ DONE (2026-04-23) — `scripts/generate_golden_candidates.py` (grounded Claude-assisted generation with citation-quote validation, per-category section filtering, adversarial branch, dedup) and `scripts/review_golden_candidates.py` (interactive [a]ccept/[e]dit/[r]eject/[s]kip/[q]uit TUI with schema re-validation and sidecar state for resume). 55 unit tests using scripted OpenAI client + scripted `input()`/editor stand-ins. Running the generator against the real MWS150A manual and human-reviewing 10 seed entries is a follow-up task (needs API access).
