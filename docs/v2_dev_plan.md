...
|------|---------|---------|
| 2026-06-25 | v2.32 | **HARNESS-23 T2 (GitHub issue #144):** make the manual sub-agent emit a clean **`Not found:` decline** as soon as it concludes the manual/vehicle has no answer, instead of searching until it times out. The #107 baseline showed **all 6 adversarial runs ended `stopped_reason=timeout` with `answer_quality=0`** ("The agent did not produce a final answer within the budget") — the agent declined correctly *in substance* but never *finalized* a decline. Two layers, both in the agent (prompt + loop, no metric/judge/golden change): **(1) Prompt** (`manual_agent_prompts.py`) — a new "When to decline early" section instructs the agent to return the documented Not-found shape *immediately*, with no further tool calls, once it sees no manual matches the vehicle (`list_manuals` has no `vehicle=`/`factory_code=` match) or the answer is absent from the section(s) that would contain it; states that an early honest decline is a CORRECT outcome. **(2) Deterministic backstop** (`manual_agent.py` `run_manual_agent`) — a no-progress guard tracks tool-call signatures; an iteration whose every call is byte-identical to one already made is "redundant". After the **first** redundant iteration it injects a one-time nudge (`_DECLINE_NUDGE`) to decline now; if the agent spins again to `_MAX_REDUNDANT_ITERATIONS` (2) the loop **force-finalizes** a `Not found:` summary (the agent's own decline if it already wrote one, else `_FORCED_DECLINE_SUMMARY`) with **`stopped_reason="complete"`** rather than grinding to the wall-clock timeout. Pairs with **T5 (#146)** which makes the judge credit a correct decline; feeds the **#155** re-baseline. Agent-only; no re-baseline (gated at #155). **Tests**: `TestRunManualAgentDeclineBackstop` + `TestForceNotFoundFinalize` in `tests/harness_agents/test_manual_agent.py` (repeated calls → complete decline well before the iter cap; nudge precedes the force; distinct reads / self-decline are not overridden); existing `test_max_iterations_exits_cleanly` updated to issue *distinct* reads so it still exercises the genuine iter-cap path. Follow-up plan: `docs/harness_14_phase6_followups.md` (T2). |
