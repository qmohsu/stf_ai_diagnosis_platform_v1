...
| **Version** | v1.0 |
| **Last updated** | 2026-05-20 (HARNESS-17: citation scroll-to-quote — fixes #101) |
...
Phasing:
1. **Scaffolding** ✅ DONE (2026-05-17, 8 commits): schema extensions, `metrics_obd.py` (TDD: 46 tests first), lane dispatcher + weight rebalance, `obd_runner.py` + adapter, judge OBD-lane sanity tests, `compute_yamaha_reference.py` developer aid, eval entry point + 3 dummy goldens + Yamaha session fixture + conftest mocks, V2 doc updates. 91 new tests; full plumbing run green via `pytest -m eval --run-eval --mock-agent --mock-judge`.
2. **Real goldens** — 10–15 hand-authored Yamaha entries covering all 6 OBD question types. No code changes — only `golden/v1/yamaha_road_test.jsonl` populated against the reference-stats script's output.
3. **Baseline + iterate** — `docs/harness_21_phase5_baseline.md` documents local Qwen + GLM 5.1 numbers; threshold raised from 0.6 once baseline known; optional `obd_agent_prompts.py` tweaks driven by failures.
