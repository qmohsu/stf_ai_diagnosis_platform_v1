...
| OBD 拒答方式适配（PROD-10 D5） | PROD-10 合并后 | OBD 提示词写明"日志无证据时回答 no evidence、不附信号 / DTC 引用"，单开小 PR，用新门槛衡量（预计收回 adversarial 题约 0.3） | 否 | 暂缓 |
...
| OBD 请求上限尾部（PROD-10） | PROD-11 真实诊断里出现"用满 20 次请求" | adversarial-002 两遍基线都用满 20 次请求（一直在找不存在的证据）；届时评估提高上限或在提示词里给"找不到就停"的步数 | 否 | 暂缓 |
