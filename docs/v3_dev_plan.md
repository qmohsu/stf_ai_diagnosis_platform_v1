...
| **Ticket 前缀** | `PROD-XX` |
| **版本** | v1.15（PROD-11 已完成：诊断任务、过程直播与回放、按需模型控制器） |
| **作者** | Xiangzhu Yan |
| **最后更新** | 2026-09-24 |
...
#### PROD-11 — 诊断 job、SSE 与回写、回放 — **已完成（2026-09-27，PR #252；学生 Swagger 走查 T-25 待外部）**
...
**前提（2026-09-24 用户决定）**：vLLM 按需拉起、不常驻（§4"vLLM 常驻与共享服务器"）——开工第一轮须先定诊断请求在 vLLM 停机时如何获得模型（排队 + 按需拉起，或同模型 API 后备），以及冷启动约 10 分钟期间用户看到什么。
**开工前三轮审核（2026-09-24 ~ 25，`.lavish/prod11_plan_review.html`）**：D1 = A（vLLM 停机时自动按需拉起：两张卡都空才起、被占就等不抢，空闲 30 分钟自动停，只停自己拉起的）；D2 = A（过程里推送第 11 种事件 `waiting` 说明在等什么，每分钟一条，自点击起最多 60 分钟，超时"模型暂不可用"）；D3 = A（真库另建与真车队隔离的测试车队，由负责前端的学生照 Swagger 走一遍，测试车队留给前端对接）；D4 = A（同一辆车同时只有一个未结束的诊断，重复点击返回原会话）。第二轮 54 条失效模式（盲审 45 + 代码细读 9）全按推荐：处理 47、推迟 2（FM-42 / FM-43，见 §4）、接受 5；第三轮 27 条测试（CI 21 条脚本化假模型、服务器 6 条）。
**服务器核验发现（2026-09-26，均已在本分支修复并回归测试）**：① 队列 worker 只导入自己任务的模块，收尾事务找不到用户 / 车间表（NoReferencedTableError），会话卡在"进行中" 6 小时——worker 启动时导入全部表，另加全新进程回归测试；② vLLM 的 conmon 落在 GPU worker 服务的 cgroup 里，部署后重启 worker 会连带杀掉 vLLM——拉起改走 `systemd-run --user --scope`；③ 有人约每小时从 talon 账号 `podman stop stf-vllm`（来源不明，用户也不知道），控制器每次又拉起——自己拉起的 vLLM 被外部停掉时标失败 + 冷却 15 分钟，等待中的诊断以新错误码 `model_stopped` 结束；④ 控制器按单个进程判断占用，别人 9 个 0.6–0.9 GB 的小进程被当成"空闲"，15:24 把 vLLM 压在别人的任务上——改为每张卡合计；⑤ 一键诊断真实路试日志三次里两次撞 100 万总 token 上限，"部分报告"只剩一句没说完的话（同款 Qwen 经 OpenRouter 复现，用户批准 C 方案）——用户选 A：闸门后加一轮不带工具的收尾报告、vLLM 档总 token 上限调到 200 万、局限一栏写成报告语言的句子（放宽上限的一次实测：11 分钟自行完成，87 万 token，报告 4317 字、19 处引用）；⑥ PROD-10 评测留下的锁文件（评测脚本按设计不删）让控制器以为评测一直在跑，vLLM 空闲 37 分钟仍不停——改为锁里写的评测容器仍在运行才算锁住（与评测脚本同一规则）。
...
| 并发诊断排队（PROD-08 FM-11） | PROD-11 建诊断队列时 | 诊断 job 并发设为 1（单模型串行），每次模型调用耗时已在事件里；出现真实并发需求再评估模型服务并发 | 否 | 已处理（PROD-11：诊断任务共用锁 `diagnosis-model`，容器 worker 一次只跑一个，其余排队；CI 测试锁住） |
...
| OBD 拒答方式适配（PROD-10 D5） | PROD-10 合并后 | 根因：V2 提示词里"总结里提到的 DTC 都要引用"与"无证据拒答不附引用"两条规则在拒答时互相矛盾，Qwen3.6 选了前者（把看过的转速和两个解不开的雅马哈 DTC 当引用）；另一题（adversarial-002）一直分析含义不明的原始氧传感器信号直到用满请求。提示词改为：拒答规则明确优先、给出拒答样例、发现阶段后先判定能不能答（拒答最多再调一次统计）、问到具体 DTC 时引用它。单开小 PR，用新门槛衡量（预计收回 adversarial 题约 0.3） | 否 | 已处理（2026-09-24，PR #250）：本地门槛评测手册 0.857 / OBD **0.945**（基线 0.885）通过；拒答题不再附信号引用（adversarial-001 0.58 → 0.94、-002 0.62 / 0.73 → 0.94、-003 → 0.99）。残留：本地仍在两道拒答题上附两个雅马哈故障码（各约 −0.06；同模型 API 预检里没有），不再单独处理，出现在真实诊断报告里再看。同模型 API 预检与本地门槛评测交叉对照：OBD 均分 0.948 / 0.945，逐题平均差 0.025（本地同代码两遍为 0.046），相关 0.86；引用习惯与请求数有稳定差异（本地附雅马哈故障码、请求更多）——API 预检可作改提示词时的快速预估，合并仍以本地为准 |
| vLLM 常驻与共享服务器（PROD-10） | 开始稳定对外提供服务；或服务器硬件升级（见下方"服务器升级"一行） | **已决策（2026-09-24，用户）：按需拉起，不常驻。** 评测 / 验证 / 诊断需要时，先确认两张卡空闲（**2026-09-27 用户放宽：别人每张卡合计 < 6 GB 即可**——Martin 的 AI 代理整晚一批接一批地跑 1–9 GB 的小任务，"两卡全空"会让诊断与评测长时间等待；vLLM 占 36.9 GB，对方留约 3 GB 余量）再 `vllm_ctl.sh start` + `wait`（冷启动约 10–12 分钟），用完 `vllm_ctl.sh stop`；不装开机自启单元。vLLM 停着时部署核验第 9 项按原因跳过（`LLM_CHECK=skip`），这是常态而非故障。PROD-11 开工时须回答"vLLM 停机时诊断请求怎么拿到模型"（排队 + 按需拉起，或同模型 API 后备 `qwen-openrouter`，后者涉及数据出境与费用） | 否 | 已决策 |
| 学生 Swagger 走查（PROD-11 T-25） | 负责前端的学生有空时 | 测试车队 + 邀请码已在服务器备好（600 权限文件，不进仓库）；学生只凭 Swagger 走 注册 → 建车 → 上传 → 诊断 → 读报告，记录卡住的地方改说明；走通后在 PROD-11 条目补"评审者验收通过" | 否 | 等待外部 |
| golden 塌方线过敏（PROD-11） | 下次改受管路径的 PR 之前 | PROD-11 两次门槛评测均值都过线（手册 0.862 / 0.852，OBD 0.896 / 0.871），但每次各有 1–2 题偶然跌破 0.4、且每次是不同的题（lookup-006、compound-001 → lookup-004、adversarial-002；这些题在另外两次都正常），另有一次与其他租户抢算力整体慢 1.6 倍；用户加 `eval-exempt` 合并。改为"同一道题在最新两份里都跌破 0.4 才算失败"之类的判定，并在成绩单里记录显卡是否被他人占用 | 否 | 暂缓 |
| OBD 请求上限尾部（PROD-10） | PROD-11 真实诊断里出现"用满 20 次请求" | adversarial-002 两遍基线都用满 20 次请求（一直在找不存在的证据）；届时评估提高上限或在提示词里给"找不到就停"的步数。D5 小 PR 已在提示词里加"发现阶段后先判定、拒答最多再调一次统计"，看它的门槛评测里 adversarial-002 是否还用满 | 否 | 暂缓（本地门槛评测：adversarial-002 请求 20 → 10，但 adversarial-001 用了 19 次，尾部仍在；按原触发条件在 PROD-11 真实诊断里看） |
| 云端对照 OBD 工具参数（PROD-10） | 需要云端 OBD 对照时 | deepseek-v3.2 反复写错 `get_signal_stats` 的 `include` 参数格式，4 题出错；放宽参数格式或在描述里给例子 | 否 | 暂缓 |
| 服务器升级到 4 × RTX PRO 6000（2026-09-24 获悉，约两个月内） | 升级排期确定时 | ① 与管理员谈卡的分配；硬件升级是"vLLM 常驻"决策的回头条件之一，届时重新评估常驻；② 升级前确认软件栈支持 Blackwell：驱动与 CUDA 12.8 以上、vLLM 镜像、宿主机 MinerU 的 PyTorch、V1/V2 的 Ollama、Podman 的 GPU 插件，预留 V3 停机窗口；③ 同一模型先在新卡上跑一轮 golden（vLLM 部署文件在门槛管理路径里，改它的 PR 会被强制评测），27B 可改为一卡一份以提高并发；④ 换更大的模型走基线重置 PR，先逐个候选跑 golden，再按新模型重校子代理预算 | 否 | 暂缓 |
| 测试车队刷诊断挤占真用户（PROD-11 FM-42） | 测试车队的诊断量影响到真车排队，或 vLLM 因联调长时间不停 | 先在联调说明里约定用法；真出现时加一个全局排队上限（例如测试车队同时最多 1 个排队），或给测试车队单独的空闲停机时间 | 否 | 暂缓 |
| V3 拖累共享 Postgres 连接数（PROD-11 FM-43） | V1/V2 容器重新启用，或出现数据库连接数告警 | 给 V3 的运行时账号设连接上限（`ALTER ROLE stf_v3_app CONNECTION LIMIT …`）并压测：直播每次轮询已是借连接即还（FM-36） | 否 | 暂缓 |
...
| v1.15 | 2026-09-27 | **PROD-11 完成**（PR #252，交付核对板通过；golden 门槛两次均值过线、塌方线各有 1–2 题随机跌破，用户加 `eval-exempt` 合并）。服务器核验发现并修复 6 个问题（worker 表注册、vLLM 独立 scope、外部停机、按卡合计、撞上限后的收尾报告 + 200 万 token、评测锁残留）；用户决定 vLLM 在别人每卡 < 6 GB 时即可拉起。§4 新增"学生 Swagger 走查"（等待外部）与"golden 塌方线过敏"（暂缓）两行 |
| v1.14 | 2026-09-25 | PROD-11 开工：三轮审核（D1–D4 = A；54 条失效模式；27 条测试）写入 PROD-11 条目；§4"并发诊断排队"改为已处理（诊断任务共用锁）；新增 FM-42（测试车队刷诊断）、FM-43（共享 Postgres 连接数）两行回头条件。2026-09-26 补服务器核验五项发现与修复（PROD-11 条目） |
| v1.13 | 2026-09-24 | **vLLM 按需拉起、不常驻**（用户决定）：§4"vLLM 常驻与共享服务器"由待决策改为已决策，回头条件为开始稳定对外服务或硬件升级；"服务器升级"一行注明它是常驻决策的回头条件；PROD-11 条目加前提（开工先定 vLLM 停机时诊断请求如何获得模型）。运维手册 v0.7（§4 按需为常态、不装开机自启单元、重启顺序不再先起 vLLM）、设计文档 v1.14、CLAUDE.md、`vllm_ctl.sh` 注释同步；部署策略变化，架构不变，**图无需改动** |
| v1.12 | 2026-09-24 | PROD-10 后续小 PR（D5，PR #250）：OBD 小助手提示词适配新模型的拒答方式——拒答规则明确优先于"总结里的 DTC 都要引用"、给出拒答样例、发现阶段后先判定能否回答（拒答最多再调一次统计）、问到具体 DTC 时引用它；新增单元测试 6 条；门槛评测结果见 §4 该行。§4 新增"服务器升级到 4 × RTX PRO 6000"回头条件，"OBD 请求上限尾部"记提示词侧处理。修正 CLAUDE.md 与 V3 部署文件注释里的迁移命令：迁移服务带 `migrate` profile，原写法在 podman-compose 1.5 下只打警告不执行（缺 `--profile migrate`）。运维手册 v0.6（基线重置示例的 OBD 验收线改为 0.884）。显卡被占时的同模型 API 预检：模型适配档新增 `qwen-openrouter`（同一 Qwen3.6-27B 经 OpenRouter，用 OpenRouter 的 `reasoning.enabled=false` 关思考、可固定托管方、预算同本地），评测命令行加 `--cloud-model` / `--cloud-provider`；预检成绩单记为云端、不计入门槛（设计文档 v1.13，图无需改动）。本地门槛评测（提交 2fd63fb）：手册 0.857、OBD 0.945，过线；手册两题波动（cross-005、image-004，手册代码未动） |
