...
| **Ticket 前缀** | `PROD-XX` |
| **版本** | v1.11（PROD-10 进行中：golden 评测移植进 V3、CI 门槛；服务器基线待跑） |
| **作者** | Xiangzhu Yan |
| **最后更新** | 2026-09-23 |
...
#### PROD-10 — Golden 评测移植与门槛 — **进行中（代码与离线测试完成；服务器基线待跑）**
...
**预算校准（T-18，2026-09-24）**：45 题、预算放大 2 倍、六路并发，519 秒跑完，0 题被预算截断、vLLM 0 次抢占；单题耗时 95 分位 98 秒（最长 105 秒）、请求数 95 分位 13（最多 18，原上限 12 不够）。按 95 分位 × 1.5：vLLM 档子代理墙钟 180 → **150 秒**、请求上限 12 → **20 次**。

**首轮归因与修正（D4）**：校准轮手册 lane 0.764 / OBD 0.853，低于验收线。手册：30 题中 20 题答案没有引用，但读的章节是对的——Qwen3.6 常把引用写成字符串 `"<章节>: <原文>"`（V2 照抄的解析器只收对象，全部丢弃），强制收尾那一轮也常交文字或把工具调用写成文字，且不补问。修正运行时：解析器接受字符串 / 其他键名的引用（manual_id 缺失时取自读过的章节）；强制收尾后交文字也补问一次（工具仍收着，不循环）。OBD：3 道"无证据"对抗题 Qwen3.6 拒答时附了信号 / DTC 引用而被扣分，提示词与 V2 逐字相同，属换模型（V2 参考 0.938 是 qwen3.5 跑的）。
...
| v1.11 | 2026-09-23 | PROD-10 进行中（代码 + 离线测试）：golden 评测移植进 `stf_v3/evals/`（V2 打分器 / 判卷 / 拼法逐字复制 + V3 执行层）、题目与日志副本带哈希清单、服务器一次性评测容器脚本、CI `eval-gate`。三轮审核：D1–D4 + FM-3 / FM-11 两处细化；59 条 FM（处理 49 / 推迟 2 / 接受 8）；23 条测试。§4：FM-56 编号映射结案，新增 FM-16 / FM-38 两条回头条件。设计文档 v1.12（§1.7 评测门槛落地；评测是工具不是运行时架构，图无需改动） |
