...
**一句话总结**:我们用一周时间完成了手册系统的"地基更换"——从"临时解析、无质检"升级为"独立索引 + 出厂验收",四个阶段全部开发完毕,前三个已合入主线,最后的对比评测今天收尾。
...
打个比方:以前我们的"图书馆"是每次开馆时,照着装订工猜的目录页临时手抄一份索引卡;装订工猜漏的书,卡片上就写着"本馆无此书"。本周我们把它改成了**正规的图书馆流程**,共四步,其中第一步是不等大改造、先堵住出血点的"先行修复":
...
| 先行修复(已上线生产一周) | 最差题 0.483 → **0.937** | "自信说没有"的失败模式已消灭 |
...
新索引系统与现行系统正在做背靠背盲测(同样 30 道题、同一评分标准、各测两轮取平均,避免单轮波动误导):

| 场次 | 状态 | 平均分 |
|---|---|---|
| 现行系统 第 1 轮 | ✅ 完成 | 0.870 |
| 现行系统 第 2 轮 | ✅ 完成 | 0.872(两轮均值 **0.871**,极稳定) |
| 新索引系统 第 1 轮(修复前) | ✅ 完成 | 0.806(暴露 2 个可修问题,当天已修) |
| 新索引系统 第 2 轮(修复后) | ✅ 完成 | **0.877** |
| 新索引系统 第 3 轮(确认轮) | ▶️ 运行中(11:21 起,约 12:15 出分) | 【待填】 |

**初步结论(待确认轮复核)**:修复后的新索引系统(0.877)**首次超过现行系统(0.870)**,且四道历史顽固低分题全部拉升到 0.94 以上(0.483→0.977、0.578→0.943、0.612→0.977、0.646→0.942)。中位数 0.937 vs 现行 0.896,分布更紧。

**逐题对账(为什么均值只领先 0.007)**:新系统涨的六题合计约 +1.0 分(假故障码拒答 +0.35、程序完整性 +0.27 等——涨点集中在"拒答有底气""程序不丢步"这类结构化收益),但被两题抵消:

1. 一道两问题(-0.545):AI 按命名规则**猜了一个格式合理但不存在的索引编号**去查,落空后又被"参阅第 X 页"式的交叉引用行带偏。两处修法均已明确(近似匹配 + 引用行降权),属小修,已排入队列。
2. 一道机油容量题(-0.218):**内容判分 0.95(几乎全对),丢分出在评分尺子自身**——专家题的"期望阅读章节"已换成新索引编号,但评测记录的是 AI 的原始查询词(它用章节标题查询,合法且成功),两种记法对不上,该维度被误判为零。**这意味着 0.877 是被低估的保守数字**;伪影影响面正在量化,将在决策包中披露,不做事后改分。
...
- 3 个章节标题在新转换下仍无独立条目(旧问题残余),现靠"归入邻近章节 + 别名 + 全文检索"三重兜底,已列入下阶段修复清单。
- 评分尺子的"记法伪影"(见第四节)将修正为记录解析后的编号,并在切换观察期内补一轮公平口径对照。
- 扫描版 PDF(无文字层)尚未支持——现有两本手册都是原生文字版;已建占位任务,第一本扫描件到达时启动。

## 六、下一步

1. **本周内(评测收尾)**:确认轮跑完 → 两个已定位的小修(索引编号近似匹配、引用行降权)落地,用定向重跑(2 题,数分钟)单独验证,不再全量;评分伪影量化披露 + 记录逻辑修正。
2. **切换决策(决策点在管理侧)**:按第五节的"架构投资"框架给出正式建议;获批后旧系统保留观察期,随时可回退。
3. **下一阶段(Phase 4,约 2 天)**:用第二本手册(丰田 Corolla,英文)完整走一遍流水线,验证"可复用"不是口号;产出新手册接入操作手册。
4. 待办移交项:生产上传通道的转换 worker 仍在用旧流程,切换获批后一并迁移;观察期内补一轮公平口径的对照评测。
