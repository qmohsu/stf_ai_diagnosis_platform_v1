...
| **Output** | Indented heading tree with slugs + DTC quick-reference index with a **Section slug** column mapping each code to its diagnostic section |
| **Implementation** | Parses markdown headings, builds tree with `parse_heading_tree()`, extracts DTC index from Appendix, then `_augment_dtc_index()` appends each code's section slug from `_build_dtc_slug_map()` (scans ALL heading depths for DTC tokens — the `故障代碼編號 PXXXX` sections sit at `####`, below the default TOC `max_depth=3`, and were otherwise unreachable; HARNESS-28, #210) |
| **File** | `harness_tools/manual_tools.py` |
...
| Field | Value |
|-------|-------|
| **Purpose** | Literal (grep-style) full-text search over one manual — the mandatory absence-check (HARNESS-30a, #217) and the locator for content the TOC does not surface (e.g. quick-index rows marked `NOT-INDEXED(use search_manual_text)`) |
| **Input** | `{ "manual_id": "MWS150A_Service_Manual", "query": "P0335", "max_hits": 20 }` |
| **Output** | Total match count + up to `max_hits` matching lines, each attributed to its enclosing section slug for direct `read_manual_section` follow-up; explicit `0 matches … the manual does not contain this text` statement on zero hits |
| **Semantics** | Case-insensitive substring — exact text, not meaning. Deliberately distinct from the semantic `search_manual` (RAG) removed in HARNESS-15: literal navigation, no embeddings, no noise on identifier queries |
| **Discipline** | The manual sub-agent's system prompt makes this a HARD gate: no "the manual does not contain X" claim without a 0-match result for X |
| **File** | `harness_tools/manual_tools.py` |
...
### 8.4 Tiered model routing (design principle, est. 2026-08-01)

Empirical foundation — the SOTA ceiling experiment (`docs/reports/2026-08-01-sota-ceiling-experiment.md`): with the code frozen and only the manual sub-agent's model swapped, four models spanning **two orders of magnitude of parameters** (local 27B dense → GLM-5.2 744B → Kimi K3 2.8T) score inside one judge-variance band (0.876–0.889) on the 30 locked goldens, while cost per full run spans **zero (local) to ~$6-8 (K3)**. Chronic low entries reproduce byte-identically across models — the residual headroom lives in golden anchor conventions and rubric calibration, not model capability. The only scale-monotonic signal is adversarial judgment (0.743 → 0.818).

Principles derived:

1. **Capability lives in the scaffolding.** The index track (HARNESS-30), tool discipline, and prompt gates reduced manual navigation to a task a 27B model saturates. Investment in tools/context engineering upgrades every model plugged in; parameter spend cannot compensate for weak scaffolding (the #135 P00AF failure was the counterexample era).
2. **Route narrow retrieval-grounded sub-agent work to the cheapest adequate model.** Sub-agent delegation (`delegate_to_manual_agent`, future OBD/subsystem agents) should carry a per-lane model assignment, chosen by bake-off data, not by default inheritance from the orchestrator.
3. **Spend model capability where scale measurably pays**: orchestrator-level judgment (tier classification, delegation decisions, refusal/escalation calls — the adversarial-type signal), out-of-distribution robustness, and cold-start exploration of new task domains before their scaffolding exists.
4. **Every new lane gets a bake-off before its model is pinned.** The eval harness produces a full acceptance verdict in 4–6 min/model (hosted) or ~11 min (local vLLM) at precisely-known cost; routing tables are filled by measurement, not intuition.
...
**Track-aware golden selection (#234).** Since the HARNESS-30 index cutover the manual agent cites `{subsystem}-{type}-{slug}` node ids, so the manual lane must be graded against the makeup overlay (`v2/locked/mws150a_indexed.jsonl`, positional-field remap of the locked file produced by `manual_pipeline/golden_makeup.py`). `resolve_manual_golden_path()` (evals conftest) selects it whenever `MANUAL_INDEX_TRACK` ≠ `off` and the overlay exists, else the legacy-slug `mws150a.jsonl` — mirroring the manual tools' own track switch. The RAG lane always grades against the legacy-slug file: pgvector chunk metadata never moved to node ids, so overlay anchors would structurally zero its `section_recall`. (#234 post-mortem: this selection previously existed only as server-side file state during the 07-28/29 A/B and baseline runs; once the checkout was cleaned, node-id citations were silently graded against legacy slugs, faking a 0.891→0.790 regression while `answer_quality` actually rose.)
...
**Pipelined execution (HARNESS-31, #225).** Since HARNESS-31 the manual and RAG lanes no longer execute inside their parametrised test bodies. A session-scoped `pipeline_results` fixture (in `tests/harness/evals/conftest.py`) reads the post-`-k` item list, builds one work item per selected golden (run callables from `lanes.py`), and hands them to `orchestrator.py`: system runs execute under an `EVAL_RUN_CONCURRENCY` semaphore (default 1 — strictly serial GPU access, execution-order-identical to the pre-pipeline suite, so scores and per-run wall latencies remain comparable), while each item's judge call fires under a separate `EVAL_JUDGE_CONCURRENCY` semaphore (default 4) as soon as its run completes — judging overlaps subsequent agent runs, dropping the judge phase (~15-30 s/golden, formerly serial) off the critical path entirely (~10-15 min per full 30-golden run). The parametrised tests are thin assertion layers over the pre-computed outcomes; per-item errors are captured and re-raised by the owning test. The report JSON gains a backward-compatible `meta.pipeline` key recording the concurrency config. `run_eval.sh` exposes `--run-concurrency` / `--judge-concurrency` and passes `-s` so the orchestrator's live `[pipeline] i/N` progress lines reach the run log. `EVAL_RUN_CONCURRENCY>1` is the direction-2 knob (concurrent agent runs); the OBD lane is not yet pipelined.

**Concurrent agent runs — multi-instance (HARNESS-31 direction 2).** Ollama 0.17's `qwen35` architecture rejects batched parallel requests (the scheduler logs `model architecture does not currently support parallel requests` and forces `Parallel:1`), so server-side `OLLAMA_NUM_PARALLEL` cannot deliver concurrency for the shipped model. The working mechanism is one full model copy per GPU: a second eval-only Ollama instance (same image/volume, `CUDA_VISIBLE_DEVICES=1`, port 11435, 98k ctx ≈ 42 GB — each RTX 6000 Ada holds one copy) plus `EVAL_LLM_ENDPOINTS=<url1>,<url2>`, which builds a checkout/checkin `asyncio.Queue` pool of per-endpoint deps in `runner.py`. At most one in-flight run per endpoint → every run keeps dedicated single-stream GPU latency, so scores and the 240 s wall are unaffected by concurrency (`EVAL_AGENT_WALL_SECONDS` exists as a separate widening knob for genuinely shared-GPU scenarios). Run with `--run-concurrency 2`.
