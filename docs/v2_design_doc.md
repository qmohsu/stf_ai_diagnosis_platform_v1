...
| **Last updated** | 2026-05-17 (v1.4) |
| **Primary pilot stack** | FastAPI + AsyncOpenAI (OpenRouter) + Ollama + pgvector (PostgreSQL) + Next.js |
| **New in this revision** | HARNESS-21 PR [1/3]: OBD sub-agent evaluation framework scaffolding landed (GitHub Issue #97). Parallels the HARNESS-14 manual-agent eval framework. Additive schema extensions (`SystemRunResult.obd_signal_citations` / `.obd_dtc_citations`; `GoldenEntry.expected_signal_citations` / `.expected_dtcs` / `.expected_no_evidence`; widened `GoldenQuestionType` literal with six OBD types; new `ExpectedSignalCitation` / `ExpectedDTC` models). New deterministic-metrics module `tests/harness/evals/metrics_obd.py` (`signal_recall`, `signal_precision`, `value_accuracy`, `dtc_accuracy`) with `expected_no_evidence` polarity flip. Lane dispatcher in `metrics.py` routes by `question_type`. New OBD runner `tests/harness/evals/obd_runner.py` adapts `OBDAgentResult` to `SystemRunResult` and honours `OBD_EVAL_AGENT_MODEL` env var for ceiling runs. New eval entry point `tests/harness/evals/test_obd_agent_eval.py` plus 3 dummy goldens in `golden/v1/yamaha_road_test.jsonl` proving the plumbing runs green via `pytest -m eval --run-eval --mock-agent --mock-judge`. Overall-score weights rebalanced to fit a 9th dimension (`value_accuracy` at 0.10 weight; sums to 1.0). Manual lane untouched. 91 new unit tests; 113 total eval-module tests pass. Design doc: `docs/plans/2026-05-17-harness-21-obd-eval-design.md`. Real goldens (10–15 hand-authored) land in PR [2/3]; baseline scorecard in PR [3/3]. |
