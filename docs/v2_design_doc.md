...
| **Last updated** | 2026-05-16 (v1.3) |
| **Primary pilot stack** | FastAPI + AsyncOpenAI (OpenRouter) + Ollama + pgvector (PostgreSQL) + Next.js |
| **New in this revision** | HARNESS-19: agent-native OBD investigation toolset landed (GitHub Issue #85). `read_obd_data`'s two-mode shape is replaced by **6 decomposed cognitive primitives** (`list_signals`, `read_window`, `get_signal_stats`, `find_events`, `list_dtcs`, `lookup_dtc`) plus an **OBD investigation sub-agent** (`app/harness_agents/obd_agent.py`, mirrors `manual_agent.py`) plus **2 delegation wrappers** (`delegate_to_obd_agent`, `delegate_to_manual_agent`) implementing hybrid Pattern 2 — main agent retains direct access to primitives AND can route compound investigations to specialists. New `OBDAgentResult` Pydantic contract (signal_citations, dtc_citations, raw_data auto-captured from tool excerpts, limitations, tool_trace). New Yamaha-aware raw loader (`obd_loader.py`) bypasses `format_normalizer.py` and exposes 16 `A_YAM_*` proprietary columns under original names. Yamaha-hex DTCs surfaced honestly with `search_manual` pivot (no fabricated decodings). Main-agent registry grew 5 → 12 tools; `read_obd_data` unregistered (file kept on disk one release cycle). 136 new unit tests pass; no regression in existing harness scope. Design doc: `docs/plans/2026-05-16-obd-toolset-design.md`. |
