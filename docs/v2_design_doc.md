...
**Architecture.** A thin restricted ReAct loop (max 12 iterations / 240 s wall, ~12K output tokens — raised from 8 / 120 s in HARNESS-23 T1, #143) calls only the 3 manual tools (`list_manuals`, `get_manual_toc`, `read_manual_section`) — `read_obd_data` and `search_manual` are explicitly excluded. The agent returns a structured `ManualAgentResult` (summary, citations, raw_sections, tool_trace, iterations, total_tokens).

**Graceful decline (HARNESS-23 T2, #144).** The loop is biased toward finalizing a `Not found:` decline rather than searching until the budget is spent (the first-round failure: all 6 adversarial runs timed out at `answer_quality=0` having declined in substance but never in output). Two layers: (1) the prompt instructs the agent to return the documented `{"summary": "Not found: …", "citations": []}` shape *immediately* once no manual matches the vehicle or the answer is absent from the section(s) that would contain it; (2) a deterministic **no-progress backstop** in `run_manual_agent` tracks tool-call signatures — when an iteration re-issues only calls already made (`_MAX_REDUNDANT_ITERATIONS`), it injects a one-time decline nudge and then force-finalizes a `Not found:` summary with `stopped_reason="complete"`. Pairs with the judge change (T5, #146) that credits a correct refusal. The output is graded by `z-ai/glm-5.1` via OpenRouter (temperature 0, JSON mode) against a human-reviewed golden entry (`GoldenEntry` — question, golden_summary, golden_citations, must_contain, must_not_contain, expected_tool_trace).
