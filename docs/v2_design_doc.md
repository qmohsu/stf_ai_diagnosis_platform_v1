...
**Architecture.** A thin restricted ReAct loop (max ~8 iterations, ~12K output tokens) calls only the 3 manual tools (`list_manuals`, `get_manual_toc`, `read_manual_section`) — `read_obd_data` and `search_manual` are explicitly excluded. The agent returns a structured `ManualAgentResult` (summary, citations, raw_sections, tool_trace, iterations, total_tokens). The output is graded by `z-ai/glm-5.1` via OpenRouter (temperature 0, JSON mode) against a human-reviewed golden entry (`GoldenEntry` — question, golden_summary, golden_citations, must_contain, must_not_contain, expected_tool_trace).
