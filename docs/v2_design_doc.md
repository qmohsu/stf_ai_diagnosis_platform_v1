...
| **Last updated** | 2026-04-23 (v1.2) |
| **Primary pilot stack** | FastAPI + AsyncOpenAI (OpenRouter) + Ollama + pgvector (PostgreSQL) + Next.js |
| **New in this revision** | HARNESS-14 phase 2 complete (GitHub Issue #73). GLM 5.1 LLM-as-judge (`tests/harness/evals/judge.py`) and restricted manual-search sub-agent (`app/harness_agents/manual_agent.py`) are both real: judge calls `z-ai/glm-5.1` via OpenRouter with JSON mode + retry-on-parse-failure, agent runs a 4-tool ReAct loop (`list_manuals`, `get_manual_toc`, `read_manual_section`, `search_manual`; `read_obd_data` excluded) and returns structured `ManualAgentResult` with summary, citations, raw_sections, and tool_trace. New `app/harness_agents/` package with `types.py` (production shapes) re-exported from the eval schemas module for a single source of truth. `--mock-judge` and `--mock-agent` CLI flags exercise the full pipeline without consuming LLM capacity. 54 new unit tests (21 judge + 33 agent). §12.5 expanded with implementation details. Ready for phase 3 (grounded golden generation script + human review CLI). |
