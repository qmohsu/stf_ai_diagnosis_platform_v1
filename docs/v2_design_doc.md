...
The `GoldenEntry` DB row carries a `tier` column (`candidate` | `locked`, CHECK-constrained) populated by `golden_sync.py` via path detection; `GoldenEntrySummary.tier` and `GoldenEntryDetail.tier` surface it on the API so a future dashboard lock badge needs no contract bump. Deletions from the locked tier always require a version bump (`v2/` → `v3/locked/`).
