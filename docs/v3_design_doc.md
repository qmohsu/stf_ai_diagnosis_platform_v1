...
| 文档控制 | |
|---|---|
| 版本 | **v1.12（PROD-10：Golden 评测移植进 V3 并成为合并门槛）** |
| 日期 | 2026-09-23 |
| 作者 | Xiangzhu Yan |
...
- **LLM 供给（C5；补充 2026-09-08 · D7）**：**唯一一个本地 vLLM 实例
  ...
  **落地（PROD-09，2026-09-19）**：vLLM 0.24 + `Qwen/Qwen3.6-27B-FP8`
  以独立部署文件 `infra/docker-compose.vllm.yml` 常驻（compose 项目
  `stf_llm`，两卡各占 80%，给手册转换留显存；思考在服务端关闭；
  `infra/vllm_ctl.sh` 启停，systemd 用户单元重启后自起）。V3 侧的
  **模型适配档**按地址 + 模型名自动选：`qwen-vllm`（每请求显式关思考、
  思考不回灌、工具不加严格模式）/ `qwen-ollama`（回退）/ `generic`
  （云端与其他）；预算与请求参数按档给默认值。报告落文前剥离带标签的
...
- **Golden 评测继续作为验收门槛**（改动不掉分才算过）。
  **落地（PROD-10）**：V2 的打分器、判卷与"结果 → 打分文本"拼法逐字复制进
  ...
  自动要求新鲜成绩单（均值 ≥ 基线 − 0.03、基线 ≥ 0.6 的题不跌破 0.4）。评测是
  工具，不是运行时组件。
...
| v1.12 | 2026-09-23 | PROD-10：§1.7 补"Golden 评测门槛"落地段（复制 V2 打分器、一次性评测容器、CI 门槛）。评测包与生产代码之间有 import 禁令；无新迁移、无新接口。评测是开发工具、不在运行时链路上，**图无需改动** |
