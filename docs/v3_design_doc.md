...
- **Agent 运行时落地（PROD-08，2026-09-17）**：主诊断 Agent + 手册 /
  ...
  数 / 工具数 / 总 token）与取消都以"部分报告"收尾（PROD-11 起：墙钟 /
  用量闸门触发后，主 Agent 再得一轮**不带工具**的收尾，按已查到的证据写
  报告；这一轮也失败才退回"最后一段文字"；局限一栏按报告语言写明是哪道
  闸门、上限多少）；报告 = 正文 +
...
- **诊断任务与过程直播（PROD-11，2026-09-25；OpenAPI 契约 v2）**：6 个接口——
  ...
  **不自动重跑**：卡死恢复遇到诊断任务就标出错"诊断被中断"，清扫任务关掉
  ...
  `llm` 队列（与手册转换并行，手册转换靠锁仍一次一本）：每张卡上别人
  （其他团队、我们自己的 MinerU / Ollama）合计不到 6 GB 时才拉起 vLLM
  （2026-09-27 用户决定由"两卡全空"放宽，给对方留约 3 GB 余量），加载中不重复
...
| v1.15 | 2026-09-25 | PROD-11：§1.4 补"诊断任务与过程直播"落地段（6 个接口 = OpenAPI 契约 v2；后台任务认领 → 装配 → 等模型 → 引擎 → 单一写入者落库 → 同一事务收尾；第 11 种事件 `waiting`；状态规则；诊断不自动重跑；按需模型控制器在宿主机 GPU worker 的 `llm` 队列）；§1.8 补 PROD-11 表字段修订与新表 `model_service_state`（业务表 11 张）。**图已同步**：任务队列加"启停 vLLM"、vLLM 框注明"按需拉起 · 空闲自停"、数据层加新表、Golden 框基线改为 PROD-10 实测 0.878 / 0.885、标题版本号。**2026-09-26 服务器核验后补**：闸门后的收尾轮（§1.4 Agent 运行时条目）；vLLM 档总 token 上限 100 万 → 200 万（一键诊断真实日志实测 87–102 万，golden 最大 29 万）；控制器按**每张卡合计**判断是否有人占用、自己拉起的 vLLM 放在独立 systemd scope、被外部停掉时标失败 + 冷却不硬拉。机制细化，架构不变，**图无需改动** |
