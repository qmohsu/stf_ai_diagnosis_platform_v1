...
| **Last updated** | 2026-06-09 (v5.9) |
| **Primary pilot stack** | FastAPI (diagnostic_api) + Ollama (`qwen3.5:27b-q8_0`) + Next.js (obd-ui) + pgvector (PostgreSQL) |
| **New in this revision** | APP-57 (fix): OBD log parser no longer invents `now()` for an unparseable `Timestamp` (GitHub issue #119).  The first real Yamaha upload contained a truncated, null-byte-padded final record (logger killed mid-write); `row_to_snapshot`'s silent `datetime.now()` fallback assigned that row the upload time, which became `max(all_ts)` and inflated `time_range.duration_seconds` from ~27 minutes to ~11.85 days.  A new `_parse_timestamp` helper raises `MalformedRowError` instead; `log_file_to_snapshots` skips the bad row and logs a warning, matching the behaviour the `time_series_normalizer` statistics path already had.  Validated end-to-end on the PolyU server against the real file. |
| **Previous revision** | APP-56: hybrid keyword + vector RAG retrieval (GitHub issue #18).  `rag_chunks` gains a `tsv tsvector GENERATED ALWAYS AS (to_tsvector('english', text)) STORED` column plus a GIN index — backfilled in-place by Postgres at migration time, no re-ingestion needed.  `retrieve_context` accepts a `mode` kwarg (`"vector"` default, preserves prior behaviour; `"keyword"` skips the embedding round-trip entirely; `"hybrid"` linearly fuses both with a per-query min-max-normalised `ts_rank`).  Each fusion branch over-pulls candidates (top-k × 4, cap 100) before the linear combination so a hit ranked #5 in both branches still wins over a single-branch top-1.  `/v1/rag/retrieve` exposes `mode` + `alpha` on the request body; the eval-suite RAG runner threads them through so the comparative judge can A/B the two paths against the locked golden corpus.  Designed to land exact DTC / part-number matches in the top-K without regressing natural-language recall. |
