...
| **Last updated** | 2026-05-02 (v4.8) |
| **Primary pilot stack** | FastAPI (diagnostic_api) + Ollama (`qwen3.5:27b-q8_0`) + Next.js (obd-ui) + pgvector (PostgreSQL) |
| **New in this revision** | APP-46: Per-page progress reporting during marker-pdf conversion. New `manuals.pages_processed` and `manuals.pages_total` columns (Alembic `s3t4u5v6w7x8`). The host worker monkey-patches `tqdm.update` to write `{manual_id}.progress.json` (atomic + throttled to 1.5 s) into the shared `.queue/`; the API polling loop in `_run_marker_convert` reads it on every tick and updates the row via a fresh `SessionLocal`. `ManualSummary` and `ManualStatusResponse` expose the new fields. The Service Manual Library renders `"{processed}/{total}"` while `status='converting'`, plus localized `Chunking…` / `Embedding…` pills for the later stages (EN / zh-CN / zh-TW). |
