...
| **Last updated** | 2026-07-12 (v5.16) |
| **Primary pilot stack** | FastAPI (diagnostic_api) + Ollama (`qwen3.5:27b-q8_0`) + Next.js (obd-ui) + pgvector (PostgreSQL) |
| **New in this revision** | APP-63 (GitHub issue #157, HARNESS-23 eval follow-up T13): **vehicle-scoped RAG retrieval on OBD diagnose**. `POST /v2/obd/{id}/diagnose` and the `POST /{id}/feedback/rag` retrieved-text snapshot called `retrieve_context` with no `vehicle_model` filter, so the LLM context mixed in the wrong vehicle's manual (proven: a Yamaha brake query returned Toyota Corolla content). Both call sites now pass the session's `vehicle_model` (APP-60); `None` (historical sessions) falls back to unfiltered retrieval, and an empty *filtered* result logs a structured `diagnosis_rag_retrieval_empty` / `rag_feedback_retrieval_empty` warning and degrades like the existing no-context path. Behaviourally dependent on the filtered-recall fix (#156). §8.3.6 `/diagnose` endpoint description updated here.<br><br>**Previous revision (v5.15):** APP-61 (follow-up to the HARNESS-23 baseline #107): **factory-code alias for service manuals**, so a manual is matched by its factory / manual code as well as its marketing model name. The Yamaha Tricity 155 manual is filed under `vehicle_model=TRICITY155`, but the locked goldens (and the code printed on the manual cover) call it `MWS-150-A`, so the honest agent (HARNESS-25) refused 27/30 baseline questions for lack of a matching manual. New **nullable** `manuals.factory_code VARCHAR(100)` (Alembic `0a1b2c3d4e5f`, backfills the existing Yamaha Tricity 155 row to `MWS150-A`); it is stamped into the `.md` frontmatter by `write_frontmatter_identity` and surfaced by the harness `list_manuals` tool, which now renders `factory_code="…"` and matches a vehicle filter against it. `POST /v2/manuals/upload` accepts an optional `factory_code` form field; `ManualSummary` / `ManualUploadResponse` expose it, and the web `ManualUploadForm` adds an optional Factory Code field (i18n en/zh-CN/zh-TW) with `ManualList` showing the code under the vehicle. §10.3.1 manuals schema and the manual-upload entry-point are updated here. The honest-match rule (treat a factory-code match as the SAME vehicle) lives in `list_manuals` + `manual_agent_prompts` (V2).<br><br>**Previous revision (v5.14):** APP-60 (follow-up to the P00AF Hiace finding #135): **required vehicle identity on OBD upload**. A vehicle model cannot be derived from an OBD log, so the uploader must state it — otherwise the agent has no way to know the vehicle (it reverse-reasoned a Hiace into a "Corolla" from the only same-make manual). New `obd_analysis_sessions.manufacturer` + `vehicle_model` (`VARCHAR(100)`) + `OBDAnalysisSession.canonical_name` property; existing `vehicle_id` kept. `POST /v2/obd/analyze` now requires `manufacturer` + `vehicle_model` query params (422 on blank); both are persisted on the session and stamped into `parsed_summary`. Alembic `a7b8c9d0e1f2` adds the two **nullable** columns (required at the API layer; DB-nullable so historical sessions stay valid — no backfill). The web `OBDInputForm` and the `jetson_uploader` edge agent (`--manufacturer` / `--model`, configured once per device) both supply them. §8.3.7 `obd_analysis_sessions` schema and the `/v2/obd/analyze` description are updated here. The agent-context grounding (`Vehicle: <Make> <Model> (VIN …)`) is HARNESS-26 (see `docs/v2_design_doc.md`).<br><br>**Previous revision (v5.13):** APP-59 (GitHub issue #136, follow-up to the P00AF Hiace finding #135): **required vehicle identity on service manuals**, so the harness can match a manual to the session's vehicle instead of confabulating (the first real agent run mistook a Toyota Hiace for a Yamaha scooter because the only manual content available — the Yamaha MWS150-A manual — was treated as authoritative). Shared `models_db.py` gains `manuals.manufacturer VARCHAR(100)` (required) with `manuals.vehicle_model` promoted to NOT NULL, plus `rag_chunks.manufacturer VARCHAR(100)` (nullable, indexed) stamped from the parent manual at ingestion (Alembic `f6a7b8c9d0e1`, backfills the two vault manuals — Yamaha TRICITY155, Toyota Corolla E11). `POST /v2/manuals/upload` now requires `manufacturer` + `vehicle_model` (422 on blank) and returns the canonical `"{manufacturer} {vehicle_model}"` name. The §10.3.1 RAG manuals/`rag_chunks` schema, the manual-upload entry-point description, and the `.md` frontmatter field list are updated here. The harness honest-matching behaviour (manual agent refuses to treat a non-matching manual as authoritative) is HARNESS-25 (see `docs/v2_design_doc.md`).<br><br>**Previous revision (v5.12):** HARNESS-24 (GitHub issue #127, shared-schema reflection — the feature itself is a V2 change, see `docs/v2_design_doc.md` v1.8.0): a sixth per-view feedback table `obd_agent_diagnosis_feedback` is added to the shared `models_db.py` so expert feedback on **agent**-generated diagnoses can be captured (previously impossible — the Agent AI tab posted to `/feedback/ai_diagnosis`, which rejects `provider='agent'` with HTTP 400). The §8.3.7 Database-tables list, the `GET /feedback` "6 tables" count, the `feedback/{feedback_type}` enum (now includes `agent_diagnosis`), the `diagnosis_history` provider CHECK note (now documents `'agent'`), and the `/history` provider filter are updated here for accuracy; the new endpoint `POST /v2/obd/{id}/feedback/agent_diagnosis` and the History "Agent Model" lane are detailed in the V2 docs. |
| **Previous revision** | APP-53 (cleanup): Deprecated edge snapshot transport removed after its one-release retention window.  Deleted `obd_agent/` acquisition layer (`api_poster`, `agent_loop`, `__main__`, `snapshot_builder`, `config`, `reader/`, simulation fixtures, edge `Dockerfile`, `requirements-sim.txt`, paired tests) and `infra/obd-agent.compose.override.yml` — the transport targeted `/v1/telemetry/obd_snapshot`, which was never deployed.  The active ingestion path is unchanged and untouched: `jetson_uploader` → `POST /v2/obd/analyze` (GitHub issue #76).  `OBDSnapshot` stays as the live row model of `log_parser`/`log_summarizer` (§8.1.1); GPL `python-obd` dependency dropped; `obd_agent` repackaged as analysis library + upload client (0.2.0). |

### Revision history

| Version | Date | Summary |
|---------|------|---------|
| v5.16 | 2026-07-12 | APP-63 (GitHub issue #157, HARNESS-23 eval follow-up T13) — vehicle-scoped RAG retrieval on OBD diagnose. `POST /v2/obd/{id}/diagnose` + the `/feedback/rag` snapshot now pass the session's `vehicle_model` (APP-60) to `retrieve_context`, fixing proven cross-manual contamination (Yamaha brake query returned Toyota Corolla chunks). `None` falls back to unfiltered; empty filtered results log a structured warning and degrade like the existing no-context path. Merge/deploy after the filtered-recall fix (#156). |
| v5.15 | 2026-06-21 | APP-61 (follow-up to the HARNESS-23 baseline #107) — factory-code alias on manuals. New **nullable** `manuals.factory_code VARCHAR(100)` (Alembic `0a1b2c3d4e5f`, backfills Yamaha Tricity 155 → `MWS150-A`); stamped into `.md` frontmatter and surfaced/matched by the harness `list_manuals` tool (renders `factory_code="…"`, matches a vehicle filter against it); `POST /v2/manuals/upload` accepts optional `factory_code`. Lets the honest agent match a question naming the factory code (e.g. `MWS-150-A`) to the `TRICITY155` manual instead of refusing — the root cause behind 27/30 agent refusals in the #107 baseline. Honest-match rule reflected in `list_manuals` + `manual_agent_prompts` (V2). |
| v5.14 | 2026-06-20 | APP-60 (follow-up to #135) — required vehicle identity on OBD upload. New `obd_analysis_sessions.manufacturer` + `vehicle_model` (nullable; API-required, 422 on blank), Alembic `a7b8c9d0e1f2`. `POST /v2/obd/analyze` requires `manufacturer` + `vehicle_model` query params, persisted on the session + `parsed_summary`. Web `OBDInputForm` + `jetson_uploader` (`--manufacturer`/`--model`, configured once per device, fail-loud) supply them. Agent-context grounding is HARNESS-26 (V2 docs). |
| v5.13 | 2026-06-19 | APP-59 (GitHub issue #136) — required vehicle identity on manuals. New `manuals.manufacturer` (required) + `manuals.vehicle_model` NOT NULL + `rag_chunks.manufacturer` (indexed), Alembic `f6a7b8c9d0e1` (backfills Yamaha TRICITY155 + Toyota Corolla E11). `POST /v2/manuals/upload` requires `manufacturer` + `vehicle_model`; canonical `"{manufacturer} {vehicle_model}"` identity surfaced in responses, the manual list, and the harness `list_manuals` tool (via `.md` frontmatter). Fixes the root cause of the P00AF Hiace mis-grounding (#135). Harness honest-matching is HARNESS-25 (V2 docs). |
| v5.12 | 2026-06-14 | HARNESS-24 (GitHub issue #127) — shared-schema reflection in the V1 docs; the feature is a V2 change (`docs/v2_design_doc.md` v1.8.0). Adds the sixth per-view feedback table `obd_agent_diagnosis_feedback` to the shared `models_db.py` (Alembic `e4f5a6b7c8d9`) so expert feedback on agent diagnoses is captured separately from local/premium. §8.3.7 Database-tables list + `GET /feedback` table count + `feedback/{feedback_type}` enum + `diagnosis_history` provider-CHECK note + `/history` provider filter updated for accuracy. New endpoint `POST /v2/obd/{id}/feedback/agent_diagnosis` and the History "Agent Model" lane are detailed in the V2 docs. |
| v5.11 | 2026-06-14 | APP-58 (fix): First-diagnosis-after-deploy "network error" — silent Ollama cold-load exceeds the SSE idle timeout (GitHub issue #128).  Container recreation (`down`+`up`) evicts the resident `qwen3.5:27b-q8_0`; the first `POST /v2/obd/{id}/diagnose` triggers a multi-minute cold load with no SSE traffic — APP‑41's `": thinking"` keep-alive only fires per reasoning chunk and Ollama returns none until the model loads — so the ~2 min browser → Cloudflare → Nginx idle timeout drops the stream, the disconnect cancels the load (`499`, "client connection closed before server finished loading"), and the UI shows `network error`.  **(1) Timer-based SSE keep-alive**: the harness router's `_with_keepalive` (15 s `": ping"` on silence) is promoted to a shared helper beside `_sse_event` in `obd_analysis.py` and wraps `generate_diagnosis`, `generate_premium_diagnosis`, and the harness Tier‑0 one-shot; it ticks from request start, covering the pre-first-chunk gap that `": thinking"` cannot.  `harness/router.py` drops its duplicate copy.  **(2) Startup pre-warm**: `expert/client.prewarm_local_model()` POSTs a `keep_alive=-1` minimal request to Ollama `/api/generate`, scheduled fire-and-forget in the `main.py` lifespan (non-blocking; benign on failure).  New `LLM_PREWARM_ON_STARTUP` setting (default true).  **(3) Resident model**: `OLLAMA_KEEP_ALIVE: "-1"` added to the base `docker-compose.yml` ollama service (already in the PolyU overlay); `LLM_PREWARM_ON_STARTUP` surfaced in both compose files.  `CLAUDE.md` deploy runbook gains a warm-up step after the health checks.  6 new tests in `tests/test_sse_keepalive.py`. |
| v5.10 | 2026-06-09 | APP-53 (cleanup): Removed the deprecated edge snapshot transport after its one-release retention window (opened v5.5).  Deleted the `obd_agent/` acquisition layer (`api_poster.py`, `agent_loop.py`, `__main__.py`, `snapshot_builder.py`, `config.py`, `reader/`, `fixtures/simulation_scenarios.json`, edge `Dockerfile`, `requirements-sim.txt`, four paired test files) and `infra/obd-agent.compose.override.yml`; the transport posted to `/v1/telemetry/obd_snapshot`, which never existed server-side.  GPL `python-obd` dropped — `obd_agent` is now GPL-free (single `requirements.txt`); repackaged 0.2.0 as analysis library + upload client, `httpx` added to `pyproject.toml` (previously missing).  `OBDSnapshot` retained as the live row model of `log_parser`/`log_summarizer` (§8.1.1); its stale removal notice replaced.  The active ingestion path — `jetson_uploader` → `POST /v2/obd/analyze` (GitHub issue #76) — is untouched. |
| v5.9 | 2026-06-09 | APP-57 (fix): Silent `now()` timestamp fallback corrupted `time_range` on truncated OBD logs (GitHub issue #119).  `obd_agent/log_parser.py:row_to_snapshot` fell back to `datetime.now(timezone.utc)` when a row's `Timestamp` failed to parse; on the first real Yamaha upload a null-byte-padded trailing record (Jetson logger killed mid-write) was stamped with the upload time and became `max(all_ts)`, ballooning `duration_seconds` to ~11.85 days for a ~27-minute capture.  New `_parse_timestamp` raises `MalformedRowError` (no `now()` fallback) and `log_file_to_snapshots` skips+warns, making the legacy summariser consistent with `time_series_normalizer` (which already skipped unparseable rows — so `value_statistics`/anomalies/clues were unaffected; only the legacy-path `time_range` was poisoned).  Validated on PolyU: 1 row skipped, `end` corrected to `2026-05-28T11:25:55Z`, `duration_seconds=1599`.  408 obd_agent tests pass. |
| v5.7 | 2026-05-24 | APP-56 (cont.): Keyword-branch query parsing switched from `plainto_tsquery` to a client-side OR-joined `to_tsquery`.  `plainto_tsquery` AND-joins terms; for any multi-word query the keyword CTE matched zero chunks (the bug fell out of the live A/B benchmark — `"P0117 coolant temperature sensor"` against the MWS-150-A corpus matched nothing because the implicit conjunction was unsatisfiable).  New `_build_or_tsquery` regex-tokenises the user query (alphanumeric, optional internal dot for part numbers), lowercases, joins with `|`, then the SQL passes through `to_tsquery`.  Hybrid CTE drops the keyword sub-CTE entirely when no usable tokens survive (avoids `to_tsquery('')` raising).  5 new unit tests for the tokeniser — 18 RAG tests pass. |
| v5.7 | 2026-05-24 | APP-56: hybrid keyword + vector RAG retrieval (GitHub issue #18).  Alembic `d3e4f5a6b7c8` adds `rag_chunks.tsv` (generated tsvector over the `text` column) and a GIN index `ix_rag_chunks_tsv`.  Migration is in-place and lossless — Postgres backfills the generated column synchronously during `ALTER TABLE`; the HNSW vector index and all existing chunk data are untouched.  `retrieve_context` gains `mode: Literal["vector", "keyword", "hybrid"]` (default `"vector"`, preserves prior behaviour) and `alpha: float` (default `0.5`) kwargs.  Hybrid path runs both branches as a single SQL statement (semantic CTE via `1 - (embedding <=> ...)`, keyword CTE via `ts_rank(tsv, plainto_tsquery('english', :q))`), pulls a top-k × 4 candidate pool (cap 100) per branch, min-max normalises `ts_rank` per-query so it's commensurate with the bounded cosine score, then fuses linearly (`α·vec + (1−α)·kw`).  `α=1` collapses to pure vector; `α=0` collapses to pure keyword.  Keyword-only mode skips the embedding round-trip entirely.  `/v1/rag/retrieve` exposes both kwargs; eval-suite `rag_runner.run_rag` threads them through and logs `mode` / `alpha` while keeping `system_label="rag"` so the comparative judge schema doesn't widen.  13 new unit tests cover mode dispatch, embedding-skip on keyword-only, top-k/alpha clamping, filter-clause assembly, and pgvector literal formatting.  A/B benchmark against the locked golden corpus is captured separately in the PR. |
...
| v5.2 | 2026-05-02 | APP-51: strip HTML pollution from chunk text + emphasis-wrapped titles. Audit of MWS-150-A's 1766 chunks found 80 chunks with `<span id="page-N-M"></span>` page-anchor markers leaking into RAG body text and ~7 emphasis-wrapped titles. New `_strip_empty_html` helper (matches `<tag ...></tag>` via backref, run twice for nested) applied to `Section.body` and `Section.title`. Title cleaner also strips leading/trailing markdown emphasis markers (`**`, `__`, `*`, `_`). New `_audit_chunks` runtime guard logs `ingest.pollution_detected` warnings per offending pattern (non-fatal regression alarm). Reingested via existing endpoint. |
...
•	Model server (Phase 1): Ollama (OpenAI-compatible endpoints). Default model: `qwen3.5:27b-q8_0` (dense 27B, Q8 quantization, ~30 GB VRAM). Qwen3.5 is a thinking model — internal reasoning tokens are streamed as SSE keep-alive comments during diagnosis to prevent proxy idle timeouts. `OLLAMA_KEEP_ALIVE=-1` keeps the model loaded in VRAM permanently. `AsyncOpenAI` timeout set to 300s for cold-load tolerance. **Cold-load resilience (APP‑58, issue #128):** `OLLAMA_KEEP_ALIVE=-1` only holds a model *after* it loads — a `down`+`up` deploy evicts it, so the first request still cold-loads. Two defences cover that gap: the API pre-warms the model in a background task at startup (`prewarm_local_model()` → Ollama `/api/generate` with `keep_alive=-1`, gated by `LLM_PREWARM_ON_STARTUP`, default on), and every diagnose SSE stream is wrapped with a *timer-based* keep-alive (`_with_keepalive`, 15 s `": ping"` on silence) that ticks from request start — covering the pre-first-token gap that the per-chunk `": thinking"` comment cannot.
...
- **Required vehicle identity (APP-60):** `manufacturer` + `vehicle_model` query params are **required** (422 on blank, whitespace-collapsed) — a model cannot be derived from an OBD log, so the uploader states it. Both are persisted on the session and stamped into `parsed_summary` (consumed by the agent's `build_user_message`, HARNESS-26); the canonical `"{manufacturer} {vehicle_model}"` is returned in `OBDAnalysisResponse`. `vehicle_id` (VIN/label) remains an optional query/`# vehicle_id:`-header input. A make/model mismatch on a dedup re-upload logs a warning (the stored row stays source of truth).
...
- The whole stream is wrapped with a timer-based keep-alive (`_with_keepalive`, shared with the premium and agent endpoints) that emits a `: ping` comment after 15 s of silence *from request start*, so a post-deploy Ollama cold-load — silent until the first token, before any `: thinking` flows — no longer trips the proxy idle timeout (APP‑58, issue #128)
- **Vehicle-scoped RAG retrieval (APP-63, issue #157):** the pre-stream `retrieve_context(rag_query, top_k=3)` call passes the session's `vehicle_model` (APP-60) so the LLM context comes only from the session vehicle's manual — previously unfiltered, a Yamaha brake query pulled Toyota Corolla chunks (cross-manual contamination). Sessions without a stored `vehicle_model` (historical pre-APP-60 rows) fall back to unfiltered retrieval; if the filtered query returns 0 chunks a structured `diagnosis_rag_retrieval_empty` warning is logged and generation proceeds with an empty context string (same degradation as the existing no-context path). The `POST /{id}/feedback/rag` retrieved-text snapshot (`top_k=5`) is scoped identically (`rag_feedback_retrieval_empty`). Behaviourally dependent on the filtered-recall fix (#156) — on the unfixed HNSW path the filter starves to 0 rows on a small corpus
...
- Each item includes: `id`, `session_id`, `provider` ("local"/"premium"/"agent"), `model_name`, `diagnosis_text`, `created_at`
- Optional `provider` query filter ("local"/"premium"/"agent")
- Returns 404 if session not found; returns 422 for invalid UUID

**Endpoint:** `GET /v2/obd/{session_id}/feedback`
- Returns all feedback rows across 6 feedback tables for a session, ordered by `created_at` descending
- Response: `FeedbackHistoryResponse` containing `session_id`, `items` (list of `FeedbackHistoryItem`), and `total` count
- Each item includes: `id`, `session_id`, `tab_name` (one of: summary, detailed, rag, ai_diagnosis, premium_diagnosis, agent_diagnosis), `rating`, `is_helpful`, `comments`, `created_at`, `diagnosis_history_id` (nullable), `diagnosis_model_name` (nullable), `diagnosis_created_at` (nullable)
...
- `feedback_type` is one of: `summary`, `detailed`, `rag`, `ai_diagnosis`, `premium_diagnosis`, `agent_diagnosis`
- Accepts expert feedback: rating (1-5), is_helpful (bool), optional comments, optional `diagnosis_history_id` (for ai_diagnosis/premium_diagnosis/agent_diagnosis only — validated against session + provider), plus type-specific fields (see table details below)
- **Multiple feedback per session allowed** (up to 10 per feedback type per session); returns 429 when the cap is reached
- Returns 404 if session not found

**Database tables:**
- `obd_analysis_sessions`: id (UUID PK), vehicle_id (String(50), VIN/label, indexed), **manufacturer + vehicle_model (String(100), APP-60 — required at upload, see `/v2/obd/analyze`; DB-nullable for historical rows; `canonical_name` property = `"{manufacturer} {vehicle_model}"`)**, status (indexed), input_text_hash (SHA-256, indexed, used for dedup), input_size_bytes, raw_input_file_path (String(500), relative path to OBD log file on disk), parsed_summary_payload (JSONB), diagnosis_text, premium_diagnosis_text, premium_diagnosis_model (String(200), latest model used), result_payload (JSONB), error_message, created_at, updated_at
- `diagnosis_history`: id (UUID PK), session_id (FK, indexed), provider (String(20), CHECK constraint: `'local'`/`'premium'`/`'agent'`), model_name (String(200)), diagnosis_text (Text), created_at. Append-only log of every AI diagnosis generation (local + premium + agent). Each regeneration creates a new row; session columns retain only the latest text for quick access. Note: agent generations are written to the session `diagnosis_text` column (shared with the local one-shot path); only `premium` has a dedicated `premium_diagnosis_text` column.
...
- `obd_agent_diagnosis_feedback`: id (UUID PK), session_id (FK), rating, is_helpful, comments, diagnosis_text (snapshots the agent AI diagnosis at submission time — sourced from the linked `diagnosis_history` row when present), diagnosis_history_id (nullable FK to `diagnosis_history.id`, validated against `provider='agent'`), audio_* columns (via `_OBDFeedbackMixin`), created_at. Added by HARNESS-24 (GitHub issue #127) so expert feedback on agent-generated diagnoses is captured separately from local/premium for training-data collection. (Shared DB model `models_db.py` — V2 detail in `docs/v2_design_doc.md` v1.8.0.)
...
> **Historical — never implemented; removed under APP-53 cleanup
> (v5.10, 2026-06-09).** The per-snapshot telemetry endpoint below was
> specified but never deployed; the V1 API was scoped down to RAG-only
> on 2026-03-08 before it was built.  The production ingestion path is
> end-of-trip log upload: `obd_agent/jetson_uploader.py` →
> `POST /v2/obd/analyze` (APP-53, GitHub issue #76).  The edge-side
> snapshot collector (`agent_loop`/`api_poster`/`reader/`) was removed
> from `obd_agent/` on 2026-06-09.  Section retained for design
> history only.
...
Manuals enter pgvector through one entry point: a PDF upload to `POST /v2/manuals/upload` triggers a background task that runs the full pipeline end-to-end. Since APP-59 the upload **requires** `manufacturer` + `vehicle_model` form fields (422 on blank, whitespace-collapsed); both are persisted on the `manuals` row, stamped onto every `rag_chunks` row, and written into the `.md` frontmatter, so the canonical `"{manufacturer} {vehicle_model}"` identity is what the UI and the harness `list_manuals` tool display. The previous CLI ingest script and dual PDF parser (PyMuPDF + bespoke OCR/vision/translation modules) were removed in APP-45; marker-pdf is now the only converter and produces the structured markdown that feeds both the vector store and the manual viewer.
...
- **APP-59 (issue #136)**: `manuals.manufacturer` (required) + `manuals.vehicle_model` (now NOT NULL) give every manual a structured vehicle identity; `rag_chunks.manufacturer` (nullable, indexed) is stamped from the authoritative `Manual` row at ingestion (`embed_and_insert_chunks`) so retrieval can filter by make + model. The canonical `"{manufacturer} {vehicle_model}"` (`Manual.canonical_name`) is the identity shown to the UI and the agent — not the raw PDF filename. Alembic `f6a7b8c9d0e1` backfills the two vault manuals (Yamaha TRICITY155, Toyota Corolla E11) and their chunks.
- **APP-61 (follow-up to baseline #107)**: optional `manuals.factory_code VARCHAR(100)` (nullable) holds a manual's factory / service-manual code when it differs from the marketing model name (e.g. `MWS150-A` for `vehicle_model=TRICITY155`). It is written into the `.md` frontmatter (`factory_code:`) by `write_frontmatter_identity`, so the harness `list_manuals` tool renders `factory_code="…"` and accepts it as a vehicle-match signal — a question naming the factory code resolves to the same manual instead of the honest agent refusing. `POST /v2/manuals/upload` accepts an optional `factory_code` form field; it is **not** denormalised onto `rag_chunks` (vector retrieval filters on the canonical `vehicle_model`). Alembic `0a1b2c3d4e5f` adds the column and backfills the Yamaha Tricity 155 row to `MWS150-A`.
...
- YAML frontmatter: `source_pdf`, `manufacturer` (APP-59), `vehicle_model`, `language`, `translated`, `exported_at`, `page_count`, `section_count`
